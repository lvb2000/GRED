...
#* model hyper-params
parser.add_argument("--model_type", default="Mamba", type=str)
...
def main_mamba():
    ...
    model = GPSModel(train_set[0]["x"].shape[-1], args.dim_h,10)
    ...
    for e in range(args.epochs):
        ...
        for s in range(train_steps_per_epoch):
            ...
            for i, idx in enumerate(batch_indices):
                ...
                dist_mask[i, :train_set[1][idx].shape[0], :train_set[1][idx].shape[1], :train_set[1][idx].shape[2]] = train_set[1][idx]
            # predict
            pred = model(torch.from_numpy(train_set[0]["x"][batch_indices]),
                        torch.from_numpy(train_set[0]["node_mask"][batch_indices]),
                        torch.from_numpy(dist_mask[:, :args.num_hops]))
            
            loss, pred_score = compute_loss(pred, torch.from_numpy(train_set[0]["y"][batch_indices]))
            loss.backward()
